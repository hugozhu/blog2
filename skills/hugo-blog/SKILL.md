...
date: YYYY-MM-DD
share_img: "/img/{YEAR}/{slug}.png"
tags: ["tag1", "tag2"]
---
```

**Note**: `share_img` is MANDATORY — added automatically during AUTO-ILLUSTRATION step (Step [4]). This ensures correct image display when sharing on social media (x.com, Facebook, etc.).
...
1. **Read the completed article** — understand the core theme, key metaphors, and narrative arc. Extract the `subtitle` from front matter for later use.
...
5. **Generate thumbnail** — JPG 800px width, quality 85:
   ...
   # generated by hugo AI
   ```
6. **Add `share_img` to front matter** — insert immediately after the `date` field (MANDATORY for social media sharing):
   ```yaml
   ---
   title: "标题"
   subtitle: "Subtitle"
   ...
   ---
   ```
7. **Insert into article** — place after opening paragraph(s), before `<!--more-->`, using the `subtitle` as alt text:
   ```markdown
   [![{subtitle}](/img/{YEAR}/{slug}-thumb.jpg)](/img/{YEAR}/{slug}.png)
   ```
   Example: `[![A Good Harness Does Not Care Which Agent Loop Wrote the Code](/img/2026/harness-not-pick-agent-loop-thumb.jpg)](/img/2026/harness-not-pick-agent-loop.png)`
8. **Verify dimensions** — confirm final image is landscape (width > height, ratio ≈ 1.78) before presenting to user
9. **Upload thumbnail to litterbox** for 24h shareable preview (user preference — always do this):
   ```bash
...
- **Always add `share_img` to front matter after generating illustration** — insert `share_img: "/img/{YEAR}/{slug}.png"` after the `date` field. This is MANDATORY for correct social media preview (x.com, Facebook, etc.).
- **Image alt text must be the subtitle** — use `[![{subtitle}](/img/{YEAR}/{slug}-thumb.jpg)](/img/{YEAR}/{slug}.png)` where `{subtitle}` is the exact English subtitle from front matter. Never use generic placeholders like "描述性 alt text".
...
- **Always add `share_img` to front matter** — after generating illustration, add `share_img: "/img/{YEAR}/{slug}.png"` to front matter (after `date` field). This ensures correct preview image when sharing on x.com/Twitter and other social media.
- **Use subtitle as image alt text** — the markdown image link MUST use the article's subtitle as alt text: `[![{subtitle}](/img/{YEAR}/{slug}-thumb.jpg)](/img/{YEAR}/{slug}.png)`. Never use generic text like "描述性 alt text".
