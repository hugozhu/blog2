...
**CRITICAL: Gemini API ignores the `size` parameter and always returns 2048×2048 square images** regardless of what you request. You MUST post-process with Pillow. Always verify output dimensions before inserting into the post.
...
4. **Crop to 16:9** — Gemini API always returns 2048×2048 square. MUST center-crop with Pillow:
   ...
   new_h = int(w / (16/9))  # 1152
   top = (h - new_h) // 2
   img = img.crop((0, top, w, top + new_h))  # 2048x1152
   img.save("cropped.png")
   # generated by hugo AI
   ```
 Or use `scripts/crop-to-ratio.py` (also supports `portrait`, `ultrawide`, `banner`):
   ```bash
   python3 scripts/crop-to-ratio.py original.png landscape
   ```
...
- **Gemini always returns 2048×2048 square** — API ignores `size` parameter. Always post-process with Pillow / `scripts/crop-to-ratio.py`. Blog illustrations MUST be 16:9 landscape.
...
- **Illustrations MUST be landscape 16:9** — Gemini API always returns 2048×2048 square. Always center-crop with Pillow before inserting. User has explicitly corrected this multiple times.
