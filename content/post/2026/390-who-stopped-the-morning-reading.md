...
**牌二，搜索域暗度陈仓。** 本机 DNS 配了搜索域 `hz.ali.com`，解析器会把裸名补全成 `dev2.hz.ali.com` 再查一次。这个全名下恰好只有一条 AAAA 记录：`fd00:1:30:...`——一个 IPv6 私有地址，没有对应的 IPv4。于是系统解析 dev2 的最终结果是一份拼盘：IPv4 来自我的 hosts，IPv6 来自 DNS。它用一行 getaddrinfo 打出来给我看，果真两条，齐齐整整。
