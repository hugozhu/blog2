...
时钟 1：任务内环（事件驱动）
 ...
 新 Issue ──→ Agent 加载 base Skill v1 ──→ 分类 / 打标签 / 评论
                                                  │
                          评论携带隐藏标记 <!-- oz-triage v:1 -->
                                                  │
                          维护者改标签 / 重开 / 写下纠正理由
                                                  │
                                                  ▼
...
时钟 2：改进外环（计划批量）
 ...
 确定性脚本聚合回看窗口内的反馈（默认 7 天）
          │
          ▼
 比较 Agent 当时的判断 vs 维护者事后的纠正
          │
    有重复差异？ ──否──→ 这一轮不改
          │是
          ▼
 生成最小 Skill diff ──→ PR ──→ 人审 ──→ merge
                                        │
                                        ▼
                      下一个 Issue 加载新版 Skill v2
