...
McConnell 的三大启示和 Claude Skill 编写指南之间，存在着一条清晰的传承线索：

| 《代码大全》的智慧 | 《Skill 编写指南》的实践 |
|---|---|
| 软件构建 > 编码 | Skill 构建 = 设计 + 指令 + 测试 + 迭代，而非仅仅写 Prompt |
| 职业金字塔 > 跳荷叶 | 三类 Skill 对应能力金字塔的三个层次 |
| 完全正确 > 近似正确 | 精确命名、精确触发、三维测试、确定性验证 |

二十年前，McConnell 教我们如何"构建"软件。今天，Skill 编写指南教我们如何"构建" AI 的行为模式。工具变了，但底层逻辑不变：

**卓越不是一个动作，而是一个习惯。它不来自于你使用了什么工具，而来自于你以何种纪律和深度来使用它。**

无论工具如何演变，对质量的追求、对成长的规划、对责任的担当，将永远是定义一位卓越工程师的真正标尺。如果你还没有读过 Anthropic 的 Skill 编写指南，我强烈建议你花 30 分钟通读一遍——不仅仅是为了学会写 Skill，更是为了理解 AI 时代"软件构建"的新范式。

而如果你已经读过《代码大全》，你会发现：**McConnell 二十年前写下的每一个字，在今天都有了全新的含义。**
