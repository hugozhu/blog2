...
3. **规则写了，没执行**：Skill 里白纸黑字写着原文链接协议「点一次图标后直接确定」，它还是对着 DOM 状态循环核对了近四分钟。
...
- **勾选状态恒为 false**：原文链接协议的 checkbox，DOM 的 checked 属性永远不反映真实状态，成功后回读仍是 false。纪律：**点一次图标后直接确定，以提交后的文案为准**。
