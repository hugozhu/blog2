...
上周三晚上，我躺在沙发上刷手机。钉钉里我那个写代码的机器人卡在一个 question 上——它问我要发到哪个群，我没看见。第二天早上才发现，会话已经卡死。

我随手在钉钉单聊里给它发了句：「给 event-watcher 加个超时自动取消，60 秒没答就 reject。」

十分钟后，机器人改完代码、跑完语法检查、重启了它自己、把改动 commit 推到了 GitHub。我在钉钉里收到了一条「✅ 已重启，能力：question 超时自动取消」的通知。
...
真正卡住自主化的，不是模型不够强，也不是代码没测试——是 **Agent 看不见「真实用户是怎么用它的、出了什么问题」**。你在本地 IDE 里给 Claude Code 贴一段报错，它只能看见你转述的那一条 prompt。它不知道：这个 Bug 是用户在什么场景下触发的？用户上一句说了什么？它自己上一次回复了什么？

没有这条「真实使用流」，Agent 修 Bug 就永远是「你描述问题 → 它猜着改」。 **自主性的瓶颈在环境接入，不在模型能力。**
...
第一条线解决「随时随地能调」——这是接入问题。第二条线才是这篇的核心： **当 agent 能看见完整消息流，这条流本身就是免费的、持续产生的回归测试集。** 人问 `1+1`、发图片、回答 question——每一次真实使用都是一条测试样本。Agent 修 Bug 时能看到「用户实际怎么触发这个 Bug」，不是你转述的报错。
...
区别在 **流的持久性和可观测性**。远程 shell 是一次性请求-响应，你发一条指令它回一段话，上下文用完即弃。钉钉会话是一条 **持续的、agent 自己能回看的流**——event-watcher 把 agent 的每次工具调用、每次回复都转发回钉钉，于是这条会话里同时有人的请求和 agent 的完整执行轨迹。agent 下次修 Bug 时，能回看「上一次用户是怎么触发这个 Bug 的、我当时怎么改的、改完跑通了没」。 **这条流既是输入、又是测试集、又是执行日志。** 远程 shell 传指令，钉钉传的是一个 agent 能自我参照的执行记忆。
...
图片那个用例最能说明「消息流是测试信号」：主模型是文本模型，本来只会回「看不到图片」；event-watcher 兜底用 gemini-3.5-flash 识别图片内容、注入会话，agent 才基于图片内容回出「1+1=2，等式正确」。 **用户发的图片本身就是一条测试样本，触发了能力缺口，缺口被同一条消息流补上了。**
...
很多人聊 Coding Agent 自主化，焦点都在「更强的模型、更长的上下文、更好的工具调用」。这些都对，但都不是杠杆点。

杠杆点是 **让 Agent 接入真实使用场景的消息流**。一旦接入：

- **测试信号免费且持续**——每一次真实使用都是回归样本，不用人造测试集。
- **Bug 定位从「转述」变「亲历」**——agent 看得到用户怎么触发的，不是你贴的报错。
- **自主修复有闭环**——改完能立刻在同一条消息流里验证、回执、迭代。

钉钉在这里不是「远程 shell」。远程 shell 只传指令；钉钉传的是 **带上下文的真实使用流**，agent 既被驱动、又从流里学习自己该怎么改。 **dogfood 是最高级的 harness——agent 开发它自己，每一行改动都被同一条会话验证。**

下一步我想把这套搬到团队场景：多个真人在钉钉群里 @ 机器人提需求，机器人改代码、跑测试、回群里报结果。那时候「消息流是测试信号」会更猛——真实用户行为的多样性会暴露单人测不出的 Bug。
