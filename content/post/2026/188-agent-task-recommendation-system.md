...
不是所有 Agent 都需要推荐系统。判断标准就两条：

- **任务多样性**：如果任务集中在 3-5 类（比如纯代码 / 纯写作），用规则路由 + 静态配置就够了，硬上推荐反而引入不可解释性。
- **资源池规模**：模型只有 2-3 个、沙箱选项固定时，推荐的搜索空间太小，收益打不平复杂度。

真正的拐点是：任务长尾化 + 模型/沙箱组合爆炸 + 单次任务成本敏感（比如 Opus 跑简单任务是浪费，Haiku 跑复杂任务会失败重试）。这时推荐系统的边际价值才显现，本质是在"成功率、成本、延迟"上做多目标优化。

### 关键架构（如果决定做）

```text
┌─────────────────────────────────────────────────────────┐
│  1. Task Profiler  (任务表征)                            │
│     - 意图分类  - 复杂度估计  - 工具/数据依赖识别        │
│     - 上下文规模  - 安全等级  - 时延 SLA                 │
└────────────────────┬────────────────────────────────────┘
                     ▼
┌─────────────────────────────────────────────────────────┐
│  2. Candidate Generator  (召回)                          │
│     从 (model × sandbox × toolset) 笛卡尔积中            │
│     用规则 + 向量召回出 Top-K 候选三元组                 │
└────────────────────┬────────────────────────────────────┘
                     ▼
┌─────────────────────────────────────────────────────────┐
│  3. Ranker  (多目标排序)                                 │
│     score = α·P(success) − β·cost − γ·latency − δ·risk  │
│     冷启动用启发式，有数据后切到 LTR / contextual bandit│
└────────────────────┬────────────────────────────────────┘
                     ▼
┌─────────────────────────────────────────────────────────┐
│  4. Executor + Guardrail                                 │
│     执行 + 失败兜底（自动升级到更强模型/更宽沙箱）       │
└────────────────────┬────────────────────────────────────┘
                     ▼
┌─────────────────────────────────────────────────────────┐
│  5. Feedback Loop                                        │
│     显式（用户👍👎）+ 隐式（重试、轮次、token、退出码）  │
│     → 特征仓库 → 离线训练 → 灰度上线 → A/B               │
└─────────────────────────────────────────────────────────┘
```
...
1. **反馈信号稀疏且有偏**：任务"成功"很难自动判定，纯靠用户反馈样本量不够。务必做隐式信号工程——重试次数、工具调用轮数、token 消耗、是否触发兜底，都是弱标签。
2. **探索-利用陷阱**：贪心策略会让系统永远收敛到"Opus + 全工具"这种安全但贵的组合。引入 contextual bandit / Thompson Sampling 强制探索新组合。
3. **冷启动**：新模型/新沙箱上线没有历史数据。解法是模型能力画像化（用标准任务集打分，得到能力向量），新模型直接进入向量召回。
4. **可解释性**：推荐错配时需要能解释"为什么选了这个组合"，否则用户和运维都没法 debug。建议保留 ranking 时的 feature 贡献。

### 我的建议

如果现在还在早期，先用规则路由 + 离线日志埋点跑 3-6 个月，把"任务特征 → 实际执行结果"的数据沉淀下来；等数据够了再上 contextual bandit。直接上完整推荐系统通常会因为冷启动数据不足，效果不如精心写的规则。
